  ...
  await prisma.$executeRawUnsafe(`
    ...
    BEGIN
      ...
      SELECT
        ...
        CASE
          WHEN data->>'Data Eleição' ~ '^\\d{4}-\\d{2}-\\d{2}$' THEN (data->>'Data Eleição')::date
          WHEN data->>'Data Eleição' ~ '^\\d{2}/\\d{2}/\\d{4}$' THEN to_date(data->>'Data Eleição', 'DD/MM/YYYY')
          ELSE NULL
        END,
        CASE
...
export async function POST(req: Request) {
  try {
    ...
    const filename = (file.name || 'cipa').toLowerCase();
    ...
    for (let i = 0; i < rows.length; i += chunk) {
      ...
      const sql = `INSERT INTO stg_cipa_raw (batch_id, row_no, data, source_file, imported_by) VALUES ${values}`;
    ...
    await prisma.$executeRawUnsafe(`SELECT apply_cipa_batch('${batchId}'::uuid)`);

    return NextResponse.json({ ok: true, batchId, total_rows: inserted });
  } catch (e: any) {
    console.error('[import/cipa] error', e);
